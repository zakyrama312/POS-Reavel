...
export default function LaporanPenitip({ laporanPenitip }: PageProps) {
    ...
    const filteredData = useMemo(() => {
        return laporanPenitip.filter((item) => {
            const createdAt = new Date(item.created_at);
            const from = fromDate ? new Date(fromDate) : null;
            const to = toDate ? new Date(toDate) : null;
            const matchesDate = (!from || createdAt >= from) && (!to || createdAt <= to);
            const matchesSearch = [item.nama_penitip, item.nama_produk].some((field) => field.toLowerCase().includes(searchTerm.toLowerCase()));
            return matchesDate && matchesSearch;
        });
    }, [laporanPenitip, fromDate, toDate, searchTerm]);
